...
function getTop250BookIds(callback){
    ...
    async.mapLimit(urls, 1,call, function (err, result) {
          if(err!=null){console.log(err);callback(null,err);}
          else {
...
function getDetailsById(id,callback){
 ...
 //   .timeout(5000)
    .end(function (err, sres) {      	
        ...
        result=reg.exec(infos);
        var isbn;
        if(result!=null){	 isbn=result[1].trim();}	
        ...
        if(e){            response.error(e);
                       ...
                       console.log(' search ids='+text);
                        ...
                        async.mapLimit(ids, 5,call, function (err, result) {
                            if(err!=null){console.log(err);response.error(err)}
                            else {
...
// search by key 
AV.Cloud.define('cloud_search_key',function(request,response){
    var key=request.params.key;
    console.log('cloud_search_key key='+key);
    var call=function(id,back){
            console.log('search  id='+id);
            getBookByDoubanId(id,function(re,e){// save database
                                if(re!=null){
